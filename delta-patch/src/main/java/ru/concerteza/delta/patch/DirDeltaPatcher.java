...
import static ru.concerteza.delta.common.IndexEntry.State.*;
...
public class DirDeltaPatcher {
    ...
    public void patch(File dir, ZipInputStream patch) throws IOException {
        ...
        check(index.getUnchanged(), dir);
        create(index.getCreated(), dir, patch);
        update(index.getUpdated(), dir, patch);
        delete(index.getDeleted(), dir);
    }
    ...
    private DeltaIndex readIndex(ZipInputStream patch) throws IOException {
        ...
        patch.closeEntry();
        List<IndexEntry.Unchanged> unchanged = ImmutableList.copyOf(Iterables.filter(entries, IndexEntry.Unchanged.class));
        List<IndexEntry.Created> created = ImmutableList.copyOf(Iterables.filter(entries, IndexEntry.Created.class));
        List<IndexEntry.Updated> updated = ImmutableList.copyOf(Iterables.filter(entries, IndexEntry.Updated.class));
        List<IndexEntry.Deleted> deleted = ImmutableList.copyOf(Iterables.filter(entries, IndexEntry.Deleted.class));
    ...
    private void check(List<IndexEntry.Unchanged> index, File dir) throws IOException {
        for (IndexEntry.Unchanged en : index) {
            File file = new File(dir, en.getPath());
            if(!(file.exists() && file.isFile())) throw new FileNotFoundException(file.toString());
            String sha1 = computeSha1(file);
            if(!sha1.equals(en.getOldSha1())) throw new IOException("UNCHANGED file check failed for file: " + file);
        }
    }
    ...
    private void create(List<IndexEntry.Created> index, File dir, ZipInputStream patch) throws IOException {
        for (IndexEntry.Created en : index) {
            ...
            checkState(en.getPath().equals(entry.getName()), "Index and zipstream unsynchronized, index: " + en.getPath() + ", zipstream: " + entry.getName());
            File file = new File(dir, en.getPath());
            if (file.exists()) throw new IOException("CREATED file already exists: " + file);
            String sha1 = copyStreamToFileWithDigest(patch, file);
            if (!sha1.equals(en.getNewSha1())) throw new IOException("CREATED file check failed for file: " + file);
            patch.closeEntry();
        }
    }
    ...
    private void update(List<IndexEntry.Updated> index, File dir, ZipInputStream patch) throws IOException {
        for (IndexEntry.Updated en : index) {
            ZipEntry entry = patch.getNextEntry();
            checkState((en.getPath() + ".gdiff").equals(entry.getName()), "Index and zipstream unsynchronized, index: " + en.getPath() + ", zipstream: " + entry.getName());
            File file = new File(dir, en.getPath());
            if (!file.exists()) throw new IOException("UPDATED file don't exist: " + file);
            String sha1old = computeSha1(file);
            if(!sha1old.equals(en.getOldSha1())) throw new IOException("UPDATED file check failed for old file: " + file);
            File patched = new File(dir, en.getPath() + UUID.randomUUID().toString());
            patch(file, patch, patched);
            String sha1new = computeSha1(patched);
            if(!sha1new.equals(en.getNewSha1())) throw new IOException("UPDATED file check failed for new file: " + file);
    ...
    private void delete(List<IndexEntry.Deleted> index, File dir) throws IOException {
        for (IndexEntry.Deleted en : index) {
            ...
            String sha1old = computeSha1(file);
            if(!sha1old.equals(en.getOldSha1())) throw new IOException("DELETED file check failed old file: " + file);
            boolean deleted = file.delete();
